...
class App {
    ...
    confirmDelete(message, onConfirm) {
        ...
        msgEl.textContent = message || "Voulez-vous vraiment supprimer cet élément ?";
    ...
    async renderUserAuthUI() {
        ...
        // 1. Topbar Actions (Haut à droite)
            ...
            if (user) {
                topbarContainer.innerHTML = `
                    ...
                    <div class="user-dropdown-menu" id="userDropdownMenu">
                        ...
                        <button class="dropdown-item" id="dropdownSettingsBtn"><i class="ri-settings-4-line"></i> Paramètres PME</button>
                        <button class="dropdown-item danger" id="dropdownLogoutBtn"><i class="ri-logout-box-r-line"></i> Se Déconnecter</button>
                    </div>
                `;
            } else {
                topbarContainer.innerHTML = `
        ...
        if (user) {
            ...
            if (sidebarUser) sidebarUser.textContent = 'Non connecté';
    ...
    async bindUserAuthEvents() {
        ...
        if (loginForm) {
            loginForm.onsubmit = async (e) => {
                ...
                // Vérification du blocage (Rate Limiting)
                ...
                if (lockoutUntil && Date.now() < parseInt(lockoutUntil)) {
                    ...
                    alert(`Trop de tentatives échouées. Par sécurité, veuillez réessayer dans ${remainingMinutes} minute(s).`);
                    return;
                }
                ...
                try {
                    ...
                    if (error.message.toLowerCase().includes('email not confirmed')) {
                        alert("Accès refusé : Vous devez d'abord valider votre adresse email en cliquant sur le lien reçu par message.");
                        return;
                    }
                    ...
                    if (attempts >= 5) {
                        const lockTime = Date.now() + 15 * 60000; // Bloqué pour 15 minutes
                        localStorage.setItem('auth_lockout_until', lockTime);
                        localStorage.setItem('auth_failed_attempts', '0');
                        alert("Compte temporairement bloqué (trop d'échecs). Réessayez dans 15 minutes.");
                    } else {
                        localStorage.setItem('auth_failed_attempts', attempts);
                        alert(`Erreur de connexion : Identifiants incorrects. Il vous reste ${5 - attempts} tentative(s) avant blocage.`);
                    }
                }
            };
        }
    ...
    bindEvents() {
        ...
        if (clientForm) {
            clientForm.addEventListener('submit', async (e) => {
                ...
                try {
                    ...
                    this.showToast('Client enregistré avec succès !');
                    if (this.currentView === 'clients') await this.renderClientsView();
                } catch (error) {
                    alert("Impossible d'enregistrer le client : " + error.message);
                }
            });
        }
        ...
        if (settingsForm) {
            settingsForm.addEventListener('submit', async (e) => {
                ...
                this.showToast('Paramètres de l\'entreprise mis à jour !');
            });
        }
        ...
        if (paymentForm) {
            paymentForm.addEventListener('submit', async (e) => {
                ...
                if (docId && amount > 0) {
                    ...
                    this.showToast(`Paiement de ${formatFCFA(amount)} enregistré avec succès !`);
    ...
    async updateSidebarOrg() {
        const user = store.getUser();
        if (!user) return; // Garder le texte "Non connecté"
    ...
    async renderView(view) {
        ...
        switch (view) {
            case 'dashboard':
                ...
                break;
            case 'factures':
                await this.renderDocumentsListView('invoice', 'Factures', 'Gérez vos factures et suivez les règlements clients.');
                break;
            case 'devis':
                await this.renderDocumentsListView('quote', 'Devis', 'Proposez vos devis et convertissez-les en factures en 1 clic.');
                break;
            case 'bons-de-commande':
                await this.renderDocumentsListView('purchase_order', 'Bon de commande', 'Émettez et suivez vos bons de commande clients.');
                break;
            case 'clients':
        ...
        const html = `
            <div class="page-header">
                <div class="page-title">
                    ...
                    <p>Aperçu en temps réel de votre activité commerciale et trésorerie.</p>
                </div>
            ...
            <div class="stats-grid">
                <div class="stat-card">
                    <div class="stat-info">
                        ...
                        <p>Total des règlements reçus</p>
                    </div>
                ...
                <div class="stat-card">
                    <div class="stat-info">
                        <span>Encours à Recouvrer</span>
                        <h3>${formatFCFA(totalPending)}</h3>
                        <p>Reste à payer sur factures émises</p>
                    </div>
                ...
                <div class="stat-card">
                    <div class="stat-info">
                        ...
                        <p>Devis prêts à être convertis</p>
                    </div>
                ...
                <div class="stat-card">
                    <div class="stat-info">
                        ...
                        <p>Entreprises partenaires enregistrées</p>
                    </div>
    ...
    renderStatusSelectorHTML(doc) {
        ...
        return `
            <select class="status-select ${statusInfo.badgeClass}" data-doc-id="${doc.id}" title="Cliquer pour modifier le statut en 1-clic">
                <option value="draft" ${doc.status === 'draft' ? 'selected' : ''}>📝 Brouillon</option>
                <option value="sent" ${doc.status === 'sent' ? 'selected' : ''}>📤 Envoyé(e)</option>
                ${doc.type === 'invoice' ? `
                    <option value="partially_paid" ${doc.status === 'partially_paid' ? 'selected' : ''}>⏳ Partiellement payée</option>
                    <option value="paid" ${doc.status === 'paid' ? 'selected' : ''}>✅ Payée (Réglée)</option>
                ` : ''}
            </select>
        `;
    }
    ...
    renderDocumentRowHTML(doc) {
        ...
        return `
            <tr>
                ...
                <td><code style="font-weight:700; color:var(--color-primary);">${escapeHTML(doc.number)}</code></td>
                <td>${escapeHTML(doc.client ? doc.client.name : 'Client inconnu')}</td>
                <td>${doc.issueDate}</td>
    ...
    bindTableActionEvents() {
        ...
        document.querySelectorAll('.status-select[data-doc-id]').forEach(select => {
            select.addEventListener('change', async (e) => {
                ...
                if (updated) {
                    const labels = { draft: 'Brouillon', sent: 'Envoyé(e)', partially_paid: 'Partiellement payée', paid: 'Payée' };
                    this.showToast(`Statut du document ${updated.number} mis à jour : ${labels[newStatus] || newStatus}`);
                    await this.renderView(this.currentView);
                }
            });
        });
                    ...
                    <table class="custom-table" id="documentsListTable">
                        <thead>
                            <tr>
                                ...
                                <th>Échéance</th>
                                <th>Montant TTC</th>
                                ${type === 'invoice' ? '<th>Reste à Payer</th>' : ''}
                                <th>Statut (Cliquer pour changer)</th>
                                <th style="text-align: right;">Actions</th>
                            </tr>
                        </thead>
                        <tbody>
                            ${docs.length === 0 ? `<tr><td colspan="8" style="text-align:center; padding:3rem; color:var(--color-text-muted);">Aucun document créé pour le moment.</td></tr>` : ''}
                            ${docs.map(doc => {
    ...
    async setupDocumentEditor(type, docId = null) {
        ...
        const html = `
            <div class="page-header">
                <div class="page-title">
                    <h2>${isEdit ? 'Modifier' : 'Créer un'} ${titles[this.editingDocType]}</h2>
                    <p>Formulaire réutilisable avec calculs TVA et totaux FCFA en temps réel.</p>
                </div>
                <div class="header-actions">
                    <button class="btn btn-secondary" id="cancelEditBtn">Annuler</button>
                    <button class="btn btn-primary" id="saveDocBtn">
                        <i class="ri-save-line"></i> ${isEdit ? 'Enregistrer les modifications' : 'Créer le document'}
                    </button>
                </div>
            </div>
            ...
            <form id="docEditorForm">
                <div class="card" style="padding: 1.5rem;">
                    <div class="form-grid">
                        ...
                        <div class="col-4 form-group">
                            <label>Statut</label>
                            <select class="form-control" id="docStatusSelect" ${!isEdit ? 'disabled' : ''}>
                                <option value="draft" ${doc && doc.status === 'draft' ? 'selected' : ''}>Brouillon</option>
                                <option value="sent" ${!doc || doc.status === 'sent' ? 'selected' : ''}>Envoyé / Émis</option>
                                ${this.editingDocType === 'invoice' ? `
                                    <option value="partially_paid" ${doc && doc.status === 'partially_paid' ? 'selected' : ''}>Partiellement payée</option>
                                    <option value="paid" ${doc && doc.status === 'paid' ? 'selected' : ''}>Payée</option>
                                ` : ''}
                            </select>
                        </div>
                    </div>
                ...
                <div class="card" style="padding: 1.5rem;">
                        ...
                        <table class="items-table">
                            <thead>
                                <tr>
                                    ...
                                    <th style="width: 45%;">Désignation des Prestations / Articles</th>
                    ...
                    <div style="display:flex; justify-content:space-between; margin-top:2rem; flex-wrap:wrap; gap:2rem;">
                        <div style="flex:1; min-width:300px;" class="form-group">
                            <label>Notes & Conditions Légales</label>
                            <textarea class="form-control" id="docNotes" rows="4" placeholder="Ex: Règlement par virement bancaire sur le compte CBAO Senegalese...">${doc ? doc.notes : (org.defaultNotes !== undefined ? org.defaultNotes : "Règlement à réception par virement ou chèque à l'ordre de " + org.name)}</textarea>
                        </div>
    ...
    renderItemRows() {
        ...
        container.innerHTML = this.editingItems.map((item, index) => `
            <tr>
                ...
                <td>
                    <input type="text" class="form-control item-des" data-index="${index}" value="${item.designation || ''}" placeholder="Description détaillée..." required>
                </td>
                <td>
    ...
    async saveDocumentFromEditor(existingId = null) {
        ...
        try {
            ...
            this.showToast(`Document ${saved.number} enregistré avec succès !`);
            await this.navigateTo('document-view', { id: saved.id });
        } catch (error) {
            alert("Impossible d'enregistrer le document : " + error.message);
        }
    }
    ...
    async renderDocumentView(id) {
        ...
        const html = `
            <div class="page-header no-print">
                <div class="page-title">
                    <h2>${doc.number}</h2>
                    <p>Aperçu et gestion de document officiel.</p>
                </div>
                <div class="header-actions">
                    ...
                    ${doc.type === 'invoice' && doc.status !== 'paid' ? `
                        <button class="btn btn-primary" id="markPaidQuickBtn">
                            <i class="ri-checkbox-circle-line"></i> Marquer comme Payée
                        </button>
                    ` : ''}
                    ${doc.type === 'quote' ? `
                    ...
                    ${doc.type === 'invoice' && remaining > 0 ? `
                        <button class="btn btn-secondary" id="addPaymentBtn">
                            <i class="ri-wallet-line"></i> Enregistrer un Règlement
                        </button>
                    ` : ''}
            ...
            <div class="print-container">
                <div class="print-header">
                    <div>
                        ...
                        <div class="print-legal-info">
                            ${org.address}<br>
                            Tél : ${org.phone} | Email : ${org.email}<br>
                ...
                <div class="print-addresses">
                    ...
                    <div class="address-box">
                        <h4>Destiné à (Client)</h4>
                        <strong>${escapeHTML(doc.client ? doc.client.name : 'Client inconnu')}</strong><br>
                        ${escapeHTML(doc.client ? doc.client.address : '')}<br>
                        ${doc.client ? `Tél: ${escapeHTML(doc.client.phone)} | Email: ${escapeHTML(doc.client.email)}` : ''}<br>
                        ${doc.client && doc.client.ninea ? `<strong>NINEA Client :</strong> ${escapeHTML(doc.client.ninea)}` : ''}
                    </div>
                </div>

                <div style="display:flex; justify-content:space-between; margin-bottom:1.5rem; font-size:0.875rem;">
                    <div><strong>Date d'Émission :</strong> ${doc.issueDate}</div>
                    <div><strong>Date d'Échéance :</strong> ${doc.dueDate || 'À réception'}</div>
                </div>
                ...
                <div style="display:flex; justify-content:space-between; align-items:flex-start;">
                    <div style="max-width:50%; font-size:0.85rem; color:var(--color-text-muted);">
                        ...
                        ${doc.notes ? `<p>${escapeHTML(doc.notes).replace(/\n/g, '<br>')}</p>` : 'Merci pour votre confiance.'}
                    </div>
        ...
        if (markPaidQuick) {
            markPaidQuick.addEventListener('click', async () => {
                ...
                this.showToast(`Facture ${doc.number} marquée comme Payée à 100% !`);
        ...
        if (convertToInvoiceBtn) {
            convertToInvoiceBtn.addEventListener('click', async () => {
                ...
                const newInvoice = {
                    ...
                    vatRate: doc.vatRate,
                    notes: `Facture générée à partir du ${sourceName} N° ${doc.number}.`,
                ...
                this.showToast(`Converti avec succès en Facture ${saved.number} !`);
                await this.navigateTo('document-view', { id: saved.id });
            });
        }
        ...
        if (convertToPOBtn) {
            convertToPOBtn.addEventListener('click', async () => {
                ...
                const newPO = {
                    ...
                    applyVat: doc.applyVat,
                    vatRate: doc.vatRate,
                    notes: `Bon de commande généré à partir du devis N° ${doc.number}.`,
                    convertedFromId: doc.id,
                    items: [...doc.items],
                    amountPaid: 0
                };
                ...
                this.showToast(`Devis ${doc.number} converti avec succès en Bon de Commande ${saved.number} !`);
                await this.navigateTo('document-view', { id: saved.id });
            });
        }
    }
        ...
        const html = `
            <div class="page-header">
                <div class="page-title">
                    ...
                    <p>Gestion de vos clients partenaires, contacts et coordonnées fiscales.</p>
                </div>
                <div class="header-actions">
                    ...
                    <table style="width: 100%; border-collapse: collapse; text-align: left;">
                        <thead>
                            <tr>
                                ...
                                <th style="padding: 1rem; border-bottom: 1px solid var(--color-border); background: var(--color-surface-hover); color: var(--color-text-muted); font-size: 0.75rem; text-transform: uppercase;">Total Facturé</th>
                            ...
                            ${clients.length > 0 ? clients.map(cli => {
                                ...
                                <tr>
                                    <td style="padding: 1rem; border-bottom: 1px solid var(--color-border);">
                                        <div style="font-weight: 600; color: var(--color-navy);">${escapeHTML(cli.name)}</div>
                                        <div style="font-size: 0.8rem; color: var(--color-text-muted);">${cli.ninea ? 'NINEA: ' + escapeHTML(cli.ninea) : ''} ${cli.rccm ? '| RCCM: ' + escapeHTML(cli.rccm) : ''}</div>
                                    </td>
                                    <td style="padding: 1rem; border-bottom: 1px solid var(--color-border);">
                                        <div style="font-size: 0.85rem;"><i class="ri-mail-line"></i> ${escapeHTML(cli.email)}</div>
                                        <div style="font-size: 0.85rem;"><i class="ri-phone-line"></i> ${escapeHTML(cli.phone)}</div>
                                    </td>
                                ...
                                `;
                            }).join('') : `<tr><td colspan="4" style="padding: 2rem; text-align: center; color: var(--color-text-muted);">Aucun client trouvé.</td></tr>`}
                        </tbody>
                    </table>
                </div>
            </div>
        ...
        const html = `
            <div class="page-header">
                <div class="page-title">
                    ...
                    <p>Configurez l'identité de votre PME, mentions légales (NINEA/RCCM) et préfixes de facturation.</p>
                </div>
            </div>
            ...
            <div class="card" style="padding: 2rem; max-width: 900px;">
                <form id="settingsForm">
                    <div class="form-grid">
                        ...
                        <div class="col-6 form-group">
                            <label>N° NINEA (Sénégal) ou IFU (Bénin / Togo)</label>
                            <input type="text" name="ninea" class="form-control" value="${org.ninea || ''}">
                        </div>

                        <div class="col-6 form-group">
                            <label>N° Registre du Commerce (RCCM)</label>
                        ...
                        <div class="col-12 form-group">
                            <label>Mentions Légales & Notes par Défaut (Factures & Devis)</label>
                            <textarea name="defaultNotes" class="form-control" rows="3" placeholder="Ex: Règlement à réception par virement...">${org.defaultNotes !== undefined ? org.defaultNotes : "Règlement à réception par virement ou chèque à l'ordre de " + org.name}</textarea>
                        </div>

                        <div class="col-12" style="margin-top:1rem; padding-top:1rem; border-top:1px solid var(--color-border);">
                            <h4 style="font-weight:700; margin-bottom:1rem;">Préfixes de Numérotation</h4>
                        </div>
                        ...
                        <div class="col-4 form-group">
                            <label>Préfixe Bons de Commande</label>
                    ...
                    <div style="margin-top:2rem; display:flex; justify-content:flex-end;">
                        <button type="submit" class="btn btn-primary">
                            <i class="ri-save-line"></i> Enregistrer les Paramètres
                        </button>
                    </div>
                </form>
            </div>
